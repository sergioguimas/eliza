...
export function CreateAppointmentDialog({ 
  ...
  useEffect(() => {
    if (open) {
      ...
      if (preselectedDate) {
        ...
        // Clique na visão de dia, preenche a hora
  ...
  const isWithinBusinessHours = (dateStr: string, timeStr: string) => {
    if (!settings) return true; // Se não tem config, assume que pode

    const selectedDate = new Date(`${dateStr}T${timeStr}`);
    const dayOfWeek = selectedDate.getDay(); // 0 (Dom) a 6 (Sab)

    // 1. Verifica se abre neste dia
    ...
    // Comparação simples de strings "HH:MM" funciona bem
    if (timeStr < start || timeStr > end) {
        return false;
    }

    // 3. Verifica horário de almoço
  ...
  const handlePreSubmit = (e: React.FormEvent) => {
    ...
    // Verifica horário antes de prosseguir
    if (!isWithinBusinessHours(date, time)) {
        setShowOutsideHoursAlert(true);
    } else {
        // Se estiver tudo ok, prossegue para o envio real
        performSubmit();
    }
  }
