...
export function AvailabilityForm({
  ...
  const [startTime, setStartTime] = useState(
    initialData[0]?.start_time?.slice(0, 5) || "08:00"
  );
  const [endTime, setEndTime] = useState(
    initialData[0]?.end_time?.slice(0, 5) || "18:00"
  );
  const [breakStart, setBreakStart] = useState(
    initialData[0]?.break_start?.slice(0, 5) || "12:00"
  );
  const [breakEnd, setBreakEnd] = useState(
    initialData[0]?.break_end?.slice(0, 5) || "13:00"
  );
  ...
  async function onSubmit() {
    ...
    if (result.success) {
      toast.success(
        messages.availability_updated_success ||
          `${messages.schedule_label || "Agenda"} de ${professionalName} atualizada com sucesso!`
      );
    } else {
      toast.error(result.error || messages.error_save || "Erro ao salvar agenda.");
    }
    ...
    <div className="space-y-8 p-2">
      <div className="space-y-3">
        <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-2">
          ...
          {messages.attendance_days_label || "Dias de atendimento"}
        </Label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-3">
          <Label className="text-xs font-semibold uppercase text-muted-foreground flex items-center gap-2">
            ...
            {messages.work_shift_label || "Turno de trabalho"}
          </Label>
      ...
      <div className="flex items-center justify-between pt-4 border-t">
        <p className="text-[10px] text-muted-foreground italic">
          {messages.schedule_applied_summary ||
        ...
        <Button onClick={onSubmit} disabled={loading || readOnly} size="sm">
          ...
          {actions.sync_schedule || "Sincronizar agenda"}
        </Button>
      </div>
    </div>
  );
