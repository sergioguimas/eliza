...
import { UpdateAppointmentDialog } from "@/components/appointments/update-appointment-dialog"
...
function CalendarContent({
        ...
        >
          ...
          <div className="flex justify-between items-center opacity-85 text-[10px] gap-2">
            <span className="font-semibold tracking-tight truncate">
              {isPanorama && appointment.profiles?.name
                ? appointment.profiles.name.split(" ")[0]
                : appointment.services?.title || servicoSingular}
            </span>
                  ...
                  <ContextMenu key={j}>
                    ...
                    <ContextMenuContent className="w-52">
                      <ContextMenuItem onClick={() => openCreateModal(day)}>
                        ...
                        {actions.create_agendamento || `Novo ${agendamentoSingular.toLowerCase()}`}
                      </ContextMenuItem>
                    </ContextMenuContent>
                  </ContextMenu>
                )
          ...
          {hours.map((hour) => {
            const hourAppointments = filteredAppointments.filter((apt) => {
              ...
              return (
                ...
                getRawHour(apt.start_time) === hour &&
                apt.status !== "canceled"
              )
            })
              ...
              <ContextMenu key={hour}>
                ...
                <ContextMenuContent className="w-52">
                  <ContextMenuItem onClick={() => openCreateModal(slotDate)}>
                    ...
                    {actions.create_agendamento || `Novo ${agendamentoSingular.toLowerCase()}`} às {hour}:00
                  </ContextMenuItem>
                </ContextMenuContent>
              </ContextMenu>
            )
  ...
  return (
    <div className="space-y-4">
                ...
                <SelectContent>
                  <SelectItem value="all">
                    <span className="font-semibold">
                      Todos ({messages.dashboard_title || "Panorama"})
                    </span>
                  </SelectItem>
                    ...
                    <SelectItem key={prof.id} value={prof.id}>
                      <div className="flex items-center gap-2">
                        <div
                          ...
                          style={{
                            backgroundColor: getProfessionalColor(prof.id).border,
                          }}
                        />
                        {prof.name}
                      </div>
      ...
      <ReturnPromptDialog
        open={returnPromptData.open}
        onOpenChange={handlePromptOpenChange}
        onConfirm={handleReturnConfirm}
        customerName={returnPromptData.appointment?.customers?.name}
      />
    </div>
  )
}
