...
import { ServiceRecordList } from "@/components/records/service-record-list"
...
type CustomerPageProps = {
  ...
  searchParams: Promise<{ return_check?: string; show_return_modal?: string }>
}
            ...
            {appointments.length === 0 ? (
              <Card className="border-dashed flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-muted-foreground">
                  Nenhum {agendamentoSingular.toLowerCase()} encontrado para este{" "}
                  {clienteSingular.toLowerCase()}.
                </p>
              </Card>
            ) : (
              appointments.map((apt: any) => (
                <AppointmentContextMenu key={apt.id} appointment={apt}>
                  <Card
                    ...
                    style={{ borderLeftColor: getStatusColorHex(apt.status) }}
                  >
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-lg">
                              {format(new Date(apt.start_time), "dd 'de' MMMM", {
                                locale: ptBR,
                              })}
                            </span>
                          ...
                          <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                            <div className="flex items-center gap-1">
                              ...
                              {format(new Date(apt.start_time), "HH:mm")} às{" "}
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              {apt.services?.title ||
                                `${servicoSingular} não informado`}
                            </div>

                            <div className="flex items-center gap-1">
                              <User className="h-4 w-4" />
                              {profissionalSingular}:{" "}
                              {apt.professionals?.name || "Não atribuído"}
                            </div>
                          </div>
                        </div>
                      </div>
        ...
        >
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6 gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Registros técnicos e observações.
              </p>
            </div>

            <Button variant="outline" size="sm" asChild>
              <a
                href={`/print/history/${customer.id}`}
                target="_blank"
          ...
          <div className="mb-8">
            <ServiceRecordForm
              ...
              defaultAppointmentId={return_check || null}
            />
          </div>

          <ReturnModalWrapper customerId={id} customerName={customer.name} />
