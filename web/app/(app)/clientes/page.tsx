...
export default async function ClientesPage({
  ...
  const pageDescription =
    ...
    `Gerencie o cadastro e histórico dos seus ${clientePlural.toLowerCase()}.`
  const searchPlaceholder = `Buscar ${clienteSingular.toLowerCase()}...`
  const emptyTitle =
    dict.messages?.clientes_empty_title ||
    `Nenhum ${clienteSingular.toLowerCase()} encontrado.`
  const createLabel =
    dict.actions?.create_cliente || `Novo ${clienteSingular.toLowerCase()}`
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-6">
        ...
        <CreateCustomerDialog triggerLabel={createLabel} />
      </div>
                ...
                <CardContent className="p-4 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    >
                      ...
                      >
                        {customer.name?.charAt(0).toUpperCase() || clienteSingular.charAt(0).toUpperCase()}
                      </span>
                    </div>
